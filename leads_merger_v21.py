...
print("\n-- Campaign 3: Winback (multi-sheet) ------------------------------------")
_win_dfs = []
for _ws_id, _ws_tab in WINBACK_SHEETS:
    try:
        _win_sh   = gc.open_by_key(_ws_id)
        _win_tabs = {ws.title: ws for ws in _win_sh.worksheets()}
        _win_part = gsheet_tab_to_df(_win_tabs, _ws_tab)
        if not _win_part.empty:
            _win_dfs.append(_win_part)
    except Exception as _we:
        print(f"   WARNING Winback sheet {_ws_id}: {_we}")
df_win_raw = pd.concat(_win_dfs, ignore_index=True) if _win_dfs else pd.DataFrame()
print(f"   Winback merged raw: {len(df_win_raw):,} rows from {len(_win_dfs)} sheet(s)")
